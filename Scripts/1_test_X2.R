################# Chi 2 ############################## -----------------
## Mientras mas grande sea la tabla de contingencia, mas grados de libertad
...
## Rechazo Ho, el p value es mayor a 0,05.
...
## En este caso el p value < 0,05 y demuestra asociacion significativa entre las variables.
...
#  C.2.3 Residuales de Pearson

#Si es positivo, el obs es mayor al eserado (AZUL). La pobreza en zona rural tiene un valor mayor al esperado. Si es negativo, el valor observado es menor al esperado. Y mientras mas grande sea la magnitud mas se aleja el observado del esperado.
...
mosaicplot(pobreza_t, # Tabla de contingencias a graficar
           ...
           xlab="Tipo de localidad",
           ylab="Situación",
           cex.axis=0.3, # Aumentar tamaño de etiquetas
           las=2,
	   shade=TRUE
           )
...
## Nos permite ver el tamaño de los residuales, el mas grande da -54 y significa que hay mucha menos gente pobre en la zona rural de la esperada.
...
corrplot(contrib, is.cor=FALSE) #Los q contribuyen mas al chi-cuadrado es la poblacion rural y no_pobre/vulnerable
...
chi_pobreza_u<-chisq.test(pobreza_u, p=c(0.25,0.25,0.25,0.25)) #misma cant de gente en cada porcentaje
...
# En el caso que se observen 25 mil registros en vulnerable x ingresos, pero se esperaban 23 mil existe un residuo.
...
#  Mide la probablidad de observar tablas muy extremas

fisher.test(hand_clap) # Es significativa la asociacion
