...
#   B.1.a Observando la distribucon de los datos ¿Se ve como campana? Campana centrada alrededor de un promedio
...
#La curva azul es mas chata y por lo tanto tiene mas desvio estandar que la rosa.
ggplot(altura_sexo, aes(x=altura, fill=sexo, y=..density..)) +
geom_histogram(position="identity")+
geom_density(alpha=0.04)

#No se distribuye normal
ggplot(data=usage, aes(x=users, y=..density..)) +
...
#Obtengo el limite inferior

#   Eliminando el outlier en el grupo Female: xq es el dato mas bajo

altura_sexo<-altura_sexo[!(altura_sexo$altura <inf_f),]

Female<-Female[Female>=inf_f]
...
t.test(Female, mu = 1.50, alternative="greater") # aceptamos que el promedio de altura de las mujeres es mayor
...
t.test(altura~sexo, altura_sexo, var.equal=FALSE)
